...
void draw_rectangle(int x, int y, int size_x, int size_y) {
    // printf("draw_rectangle(): Drawing rectangle size x: %d y: %d on location
    // (%d,%d)\n", size_x, size_y, x, y);
    int _x = 0;
    int _y = 0;
    if ((_x + size_x > SCREEN_WIDTH) || (_y + size_y > SCREEN_HEIGHT)) {
        panic("draw_rectangle(): x or y error: x=%d, y=%d, size_x=%d, "
              "size_y=%d, height=%d, width=%d",
...
void drawBox(int x, int y, int size) {
    ...
    int _x = 0;
    int _y = 0;
    if ((_x + size > SCREEN_WIDTH) || (_y + size > SCREEN_HEIGHT)) {
        panic(
            "drawBox(): x or y error: x=%d, y=%d, size=%d, height=%d, width=%d",
            x, y, size, SCREEN_HEIGHT, SCREEN_WIDTH);
    }
...
void fb_put(char chr[], int x, int y) {
    ...
    int mask[8] = {128, 64, 32, 16,
                   8,   4,  2,  1}; // Mask for shielding specific bits
    // printf("Print ASCII %d to framebuffer\n",*chr);
    if (*chr == 10) { // Found \n?
        // printf("\\n detected\n");
        row = 1;
        line++; // Move to first row, next line.
        return;
    }
